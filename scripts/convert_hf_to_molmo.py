...
from olmo import VisionBackboneConfig, ModelConfig, BlockType
...
def convert_state_dict_olmo_1024_preview(
    ...
    for layer in range(config.n_layers):
        ...
        mapped_layer_dict = {
            ...
            "ff_norm": {
                "weight": layer_dict["post_feedforward_layernorm"].pop("weight")
            },
            "attn_norm": {
                "weight": layer_dict["post_attention_layernorm"].pop("weight")
            },
            "att_proj": dict(
                weight=torch.cat((q, k, v), dim=0),
            ),
