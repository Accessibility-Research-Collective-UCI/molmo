...
TF_RECORD_NAMES = [f"android_control-000{str(i).zfill(2)}-of-00020" for i in range(20)]
TFRECORD_URLS = [
...
def process_data(data_files: Dict[str, str]):
    ...
    try:
        ...
        raise ImportError(
            "Unable to import android control protos, make sure https://github.com/google-deepmind/android_env"
    ...
    try:
        ...
        raise ImportError(
            "Building android control requires tensorflow to be installed"
        )
    ...
    for record in tqdm(raw_dataset, desc="Processing", total=total):
        ...
        parsed_data = {
            ...
            "goal": example.features.feature["goal"]
            .bytes_list.value[0]
            .decode("utf-8"),
            "screenshots": [
                screenshot
                for screenshot in example.features.feature[
                    "screenshots"
                ].bytes_list.value
            ],
            "screenshot_widths": example.features.feature[
                "screenshot_widths"
            ].int64_list.value,
            "screenshot_heights": example.features.feature[
                "screenshot_heights"
            ].int64_list.value,
            "actions": [
                action.decode("utf-8")
                for action in example.features.feature["actions"].bytes_list.value
            ],
            "step_instructions": [
                instruction.decode("utf-8")
                for instruction in example.features.feature[
                    "step_instructions"
                ].bytes_list.value
            ],
        }
        ...
        a11y_tree = (
            android_accessibility_forest_pb2.AndroidAccessibilityForest().FromString(
                example.features.feature["accessibility_trees"].bytes_list.value[0]
            )
        )
        ...
        for i, screen in enumerate(screenshots):
            ...
            a11y_tree = android_accessibility_forest_pb2.AndroidAccessibilityForest().FromString(
                example.features.feature["accessibility_trees"].bytes_list.value[i]
            )
            ...
            action_history.append(
                cur_action
            )  # append the current action to the action history of the next datapoint
...
class AndroidControlBuilder(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager):
        ...
        return [
            datasets.SplitGenerator(
                name=k, gen_kwargs=dict(processed_data=processed_data[k])
            )
    ...
    def _generate_examples(self, processed_data):
        for row_id, data in enumerate(processed_data):
            ...
            target_box = ""
            if target_action.startswith("Click") or target_action.startswith(
                "Long Press"
            ):
                gt_coords = re.findall(r"\d+", target_action)
                gt_coords = (int(gt_coords[0]), int(gt_coords[1]))
                bbs, bb_centers, bb_sizes, metadata = extract_bbs_from_a11y(
                    data["a11y"], dims
                )
                target_box = find_gt_box(
                    gt_coords, bb_centers, bb_sizes, bbs, strategy="smallest"
                )
                target_box = self.map_bounding_box(target_box, dims)
            elif target_action.startswith("Open App"):
                app_name = target_action[9:]
                target_box = extract_app_bb(
                    reduced_a11y, search_text="text=" + app_name
                )
                if target_box != "":
                    target_box = self.map_bounding_box(target_box, dims)
            ...
            example = {
                "question": data[
                    "instruction"
                ],  # repeat of ll_instruction for sake of vqa_preprocessor
                "answer": answer,
