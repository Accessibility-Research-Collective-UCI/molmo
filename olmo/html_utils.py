...
def example_to_html_dict(ex, preprocessor, show_patches=False, show_crops=False):
    ...
    if "metadata" in ex:
        ...
        metadata = {
            k[len("metadata/") :]: v for k, v in ex.items() if k.startswith("metadata/")
        }
    voc = preprocessor.tokenizer
    image_input_idx = ex["image_input_idx"].ravel()
    token_to_patch_ix = {
        k: i for i, k in enumerate(image_input_idx.reshape(-1)) if k >= 0
    }
    ...
    if "subsegment_ids" in ex:
        ...
        for i in np.sort(np.unique(subsegment_ids)[0]):
            ...
            segment_text.append(
                (i, voc.decode(targets[mask], False), ex["loss_masks"][mask].mean())
            )
        ...
        for i, seg, w in segment_text:
            ...
            if seg_points:
                ...
                text.append(
                    f'<span style="color: {color}">SEGMENT {i}</span> w={w:0.3f}: '
                    + escape_html(seg)
                )
                boxes.append(
                    BoxesToVisualize(
                        [[x - 5, y - 5, x + 5, y + 5] for x, y in seg_points],
                        color,
                        "xyxy",
                    )
                )
        ...
        if "image_size" in metadata:
            ...
            boxes = [
                BoxesToVisualize(
                    [[x - 5, y - 5, x + 5, y + 5] for x, y in points], "blue", "xyxy"
                )
            ]
    ...
    if image_src is not None:
        ...
        if len(boxes) == 0:
            out["image"] = (
                f'<img style="max-height:{max_dim}px;max-width:{max_dim}px;height:auto;width:auto;" src={image_src}><img>'
            )
        else:
            out["image"] = get_html_image_with_boxes(
                image_src, boxes, img_size=metadata.get("image_size"), max_dim=max_dim
            )

    base_image_input_d = 14
    images = einops.rearrange(
        ex["images"],
        "t (h w) (dh dw c) -> t (h dh) (w dw) c",
    ...
    if show_patches:
        ...
        patches = einops.rearrange(
            images, "t (h dh) (w dw) c -> (t h w) dh dw c", dh=28, dw=28
        )
        # patches = tf.transpose(patches, [0, 2, 1])
        assert len(patches) == len(image_input_idx)
        assert (ex["decoder_input_tokens"] == image_patch_id).sum() == len(
            token_to_patch_ix
        )
        on = 0
...
def html_rect(x1, y1, x2, y2, color="black", border_width="medium", label=None):
    ...
    if text:
        ...
        html = ["<div>", f'  <div style="{rect_style_str}"></div>', text, "</div>"]
    return html


def get_html_image_with_boxes(
    image_src,
    boxes: List[BoxesToVisualize],
    width=None,
    height=None,
    wrap="div",
    img_size=None,
...
def postprocess_prompt(prompt_text):
    ...
    prompt_text = (
        prompt_text.lstrip()
    )  # some tokenizers add a leading space before special tokens
    post_processed_text = ""
    for match in re.finditer(
        r"<im_start>\s?((<im_patch>|<im_col>)\s?)*\s?<im_end>", prompt_text
    ):
